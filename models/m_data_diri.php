...
defined('BASEPATH') OR exit('No direct script access allowed');
 ...
 class m_data_diri extends CI_Model{
    function data_diri_kons()
   {
    ...
    {
      $ngambil =$this->session->userdata("iduser");
      $q="SELECT * FROM keahlian2, pengajar WHERE keahlian2.id_pengajar = pengajar.id_pengajar AND pengajar.id_akun = $ngambil";
      $query = $this->db->query($q);
      return $query->result();
    }
    ...
    {
      $ngambil =$this->session->userdata("iduser");
      $q="SELECT * FROM waktu_ngajar2, pengajar WHERE waktu_ngajar2.id_pengajar = pengajar.id_pengajar AND pengajar.id_akun = $ngambil";
      $query = $this->db->query($q);
      return $query->result();
    }
