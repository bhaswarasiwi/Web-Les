 ...
 class m_pertemuan extends CI_Model{
   function tampil_jam($id_penugasan){
	...
	function data_pertemuan($id_penugasan){
		...
		$q="SELECT penugasan.id_penugasan,penugasan.id_pengajar,penugasan.id_kons, penugasan.nama_kons, pesan2.materi, pesan2.hari, pesan2.jam, pesan2.tempat_pertemuan, penugasan.tgl_mulai, penugasan.tgl_selesai FROM penugasan , permintaan_kons , pesan2 , pengajar WHERE IF(penugasan.id_pesan is not null , penugasan.id_pesan = pesan2.id_pesan AND penugasan.id_pengajar = pengajar.id_pengajar  , penugasan.id_ganti_les = permintaan_kons.id_ganti_kons AND penugasan.id_pengajar=pengajar.id_pengajar)  AND penugasan.`status` NOT LIKE 'Tidak Aktif%' AND id_penugasan = $id_penugasan GROUP BY penugasan.id_penugasan";
		$query = $this->db->query($q);
		return $query->row();
	}
